...
class ImprovedDecisionTree(object):
    ...
    def __init__(self, impurity_criterion='entropy', same_ratio=None, level=3, threshold=None):
        '''
        ...
        # categorical (or continuous)None
    ...
    def fit(self, X, y, feature_names=None):
        ...
        # This piece of code is used to provide feature names to the Decision
        # tree
        if feature_names is None or len(feature_names) != X.shape[1]:
            # if the user has not provided feature names, just give them
            # numbers
        ...
        def is_categorical(x): return isinstance(x, (str, bool))

        # Each variable (organized by index) is given a label categorical or
        # not
    ...
    def _information_gain(self, y, y1, y2):
        ...
        total -= (self._entropy(y1) * (len(y1) / len(y)) +
                  self._entropy(y2) * (len(y2) / len(y)))
    ...
    def _choose_split_index(self, X, y):
        ...
        # if there are less than 2 values, move on to the next column
            ...
            for v in values:
                temp_X1, temp_y1, temp_X2, temp_y2 = self._make_split(
                    X, y, col, v)
                temp_gain = self._information_gain(y, temp_y1, temp_y2)
                if temp_gain > max_gain:
                    max_gain = temp_gain
                    split_index = col
                    split_value = v
                    splits = temp_X1, temp_y1, temp_X2, temp_y2
    ...
    #    * if the leaf size is not None:
        ...
        if depth is not None and self.tree_level > depth:
            return True
        ...
        if self.same_ratio is not None:
            ...
            y1_ratio = Counter(y1).most_common(1)[0][1] / len(y1)
            y2_ratio = Counter(y2).most_common(1)[0][1] / len(y2)
            ...
            if y1_ratio >= self.same_ratio or y2_ratio >= self.same_ratio:
                # print(“True in y1_ratio condition, y1_ratio = “, y1_ratio)
                return True
        ...
        if self.threshold is not None:
            ...
            if self.info_gain <= self.threshold:
                # return True
                # print(“True in info_gain condition, info_gain = “, info_gain)
                return True
