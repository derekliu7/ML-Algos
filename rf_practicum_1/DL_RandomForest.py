...
class RandomForest(object):
    ...
    def build_forest(self, X, y, num_trees, num_samples, num_features):
        ...
        for sample in list_samples:
            tree = DecisionTree()
            X = sample[:, :-1]
            X[:, random.randrange(X.shape[1])] = np.zeros(len(X))
            tree.fit(X, sample[:, -1])
    ...
    def predict(self, X):
        ...
        for tree in self.forest:
            list_pred.append(tree.predict(X))
        mat = np.column_stack(i for i in list_pred)
        list_row_pred = []
        for i in range(mat.shape[0]):
            counts = Counter(mat[i])
            list_row_pred.append(counts.most_common(1)[0][0])
