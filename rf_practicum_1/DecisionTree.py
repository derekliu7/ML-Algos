...
class DecisionTree(object):
    ...
    def _make_split(self, X, y, split_index, split_value):
        ...
        if self.categorical(X[:, split_index]):
    ...
    def _choose_split_index(self, X, y):
        ...
        # if there are less than 2 values, move on to the next column
            ...
            for v in values:
                temp_X[A], temp_y[A], temp_X[B], temp_y[B] = self._make_split(
                    X, y, col, v)
                temp_gain = self._information_gain(y, temp_y[A], temp_y[B])
                if temp_gain > max_gain:
                    max_gain = temp_gain
                    (X1, y1, X2, y2) = (temp_X[A],
                                        temp_y[A], temp_X[B], temp_y[B])
